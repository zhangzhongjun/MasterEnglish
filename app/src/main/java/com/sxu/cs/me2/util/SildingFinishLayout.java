...
public class SildingFinishLayout extends RelativeLayout implements
	...
	private void scrollRight() {
		...
		System.out.println("delta = "+delta);//223  183
		System.out.println(mParentView.getScrollX());//-317  -357
	...
	private void scrollLeft() {
		...
		System.out.println("delta = "+delta);//277
		System.out.println(mParentView.getScrollX());//277
		// 调用startScroll方法来设置一些滚动的参数，我们在computeScroll()方法中调用scrollTo来滚动item
		mScroller.startScroll(mParentView.getScrollX(), 0, -delta+1, 0,
				Math.abs(delta));
		postInvalidate();
	...
	public boolean onTouch(View v, MotionEvent event) {
		switch (event.getAction()) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				} else if (moveX - downX <= 0 && isSilding) {
					mParentView.scrollBy(deltaX, 0);
			...
			case MotionEvent.ACTION_UP:
				isSilding = false;
				if (mParentView.getScrollX() <= -viewWidth / 2 && canRight) {
					isFinish = true;
					scrollRight();
				} else if (mParentView.getScrollX() >= viewWidth / 2 && canLeft) {
					isFinish = true;
					scrollLeft();
				} else {
					scrollOrigin();
